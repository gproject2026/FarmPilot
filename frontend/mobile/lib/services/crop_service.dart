...
class CropService {
  Future<List<dynamic>> getMyCrops(String token) async {
    final response = await http.get(
      Uri.parse('${AppConstants.baseUrl}/crops/my'),
    ...
    if (response.statusCode == 200) {
      ...
      if (decodedBody is! List) {
        throw Exception('Invalid crops response');
      }
  ...
  Future<Map<String, dynamic>> createCrop({
    ...
    final response = await http.post(
      Uri.parse('${AppConstants.baseUrl}/crops'),
      headers: _headers(token),
      body: jsonEncode({
        'cropName': cropName,
        ...
        if (_hasValue(irrigationScheduleEn))
          'irrigationScheduleEn': irrigationScheduleEn,

        if (_hasValue(irrigationScheduleAr))
          'irrigationScheduleAr': irrigationScheduleAr,

        if (_hasValue(fertilizationSchedule))
