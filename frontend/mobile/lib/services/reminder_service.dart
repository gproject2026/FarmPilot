...
class ReminderService {
  Future<List<ReminderModel>> getReminders({required String token}) async {
    ...
    if (response.statusCode == 200) {
      ...
      return decodedBody
          .map(
            (item) => ReminderModel.fromJson(Map<String, dynamic>.from(item)),
          )
          .toList();
    }

    throw Exception(
      _readErrorMessage(response.body, 'Failed to load reminders'),
    );
  }
  ...
  Future<ReminderModel> createReminder({
    ...
    throw Exception(
      _readErrorMessage(response.body, 'Failed to create reminder'),
    );
  }
  ...
  Future<ReminderModel> updateReminder({
    ...
    final response = await http.patch(
      Uri.parse('${AppConstants.baseUrl}/reminders/$reminderId'),
      headers: _headers(token),
    ...
    throw Exception(
      _readErrorMessage(response.body, 'Failed to update reminder'),
    );
  }
  ...
  Future<void> deleteReminder({
    ...
    if (response.statusCode != 200 && response.statusCode != 204) {
      throw Exception(
        _readErrorMessage(response.body, 'Failed to delete reminder'),
      );
    }
  }
